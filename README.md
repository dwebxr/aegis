...
- **Push notification security (2 highs closed)**:
  - `getPushSubscriptions(user)` and `removePushSubscriptions(user, endpoints)` on the canister are now `caller`-gated — empty list / trap for non-owners + non-controllers. Previously these were public queries / unauth'd updates, leaking the Web Push secret (endpoint + p256dh + auth keys are functionally equivalent to a sender capability) and allowing third-party deletion of subscriptions
  - `/api/push/token` HMAC token now binds to `(principal, endpoints)` tuple (NUL-separated, lowercase + sorted endpoints) instead of just principal. Captured tokens cannot be expanded to other endpoints, and unauthenticated callers cannot mint a usable token without already knowing valid endpoints — and after the canister fix above, the only path to learn an endpoint is to own the principal that registered it
  - `/api/push/send` rewritten to receive the subscription set from the client and stop calling the canister anonymously (which would now return empty). Cleanup of expired endpoints (410/404) is the client's responsibility via the auth'd `unregisterPushSubscription` actor call. `app/page.tsx` push flow updated to fetch subs via auth'd actor, pass them through, and clean up expired endpoints
- **x402 paywall closure**: `/api/d2a/briefing/changes` was bypassing the x402 gate that protected `/api/d2a/briefing` — exposing title/sourceUrl/composite metadata for free. Wrapped with the same `withX402` + free-tier preview pattern; preview mode redacts title and sourceUrl, leaving only itemHash + composite + generatedAt so callers learn that something changed but not which article it is
- **`/api/translate` 500→400 on bad JSON**: switched to `parseJsonBody` helper to match the rest of the routes — invalid JSON is a 400 with explanatory error instead of an unhandled 500 that just produced Sentry noise
- **8-agent parallel cleanup sweep** (Round 3):
  - **DRY**: new `lib/utils/icTime.ts` with `msToNs`/`nsToMs`/`nowNs`/`NS_PER_MS` — replaces 12+ inline `BigInt(Math.round(ms)) * BigInt(1_000_000)` and `Number(ns) / 1_000_000` sites across `contexts/content/icSync.ts`, `contexts/SourceContext.tsx`, `contexts/AuthContext.tsx`, `lib/d2a/briefingProvider.ts`, `lib/nostr/linkAccount.ts`. Unifies the precision-safer `Number(ns / NS_PER_MS)` form. New `distributedGuardAndParse()` helper in `lib/api/rateLimit.ts` mirrors the existing `guardAndParse()` — `/api/analyze` and `/api/briefing/digest` migrated
  - **Type consolidation**: inline `(msg, type: "error" | "info" | "success") => void` callback shape in `contexts/content/icSync.ts` (3 sites) replaced with `NotificationType` exported from `hooks/useNotifications.ts`. `QuickAddId` literal union in `components/tabs/SourcesTab.tsx` was a verbatim duplicate of `SourcePlatform` from `lib/types/sources.ts` — removed and consumers point at the canonical type (5 references)
  - **Strong types**: 4 `Record<string, unknown>` → concrete types — `lib/ingestion/fetchers.ts` RSS request body, `contexts/content/scoring.ts` analyze body, `contexts/SourceContext.tsx` source config payload, `lib/ic/icpLedger.ts` ICRC-2 `ApproveError` proper variant union (was `Record<string, unknown>` rendering all of `BadFee`/`InsufficientFunds`/`AllowanceChanged`/`Expired`/etc. opaque)
  - **Defensive code**: `components/settings/FeedSection.tsx::handleSaveApiKey` had a `try/catch` wrapping `setUserApiKey()` whose only throw path is the `sk-ant-` prefix check already gated above — unreachable defensive code removed (-10 lines, one duplicate "Failed to save" notification path eliminated)
  - **Unused exports (knip-verified)**: 13 type exports demoted to module-private — `HandshakePhase`, `D2AOfferMessage`, `D2AAcceptMessage`, `D2ARejectMessage`, `D2ADeliverMessage`, `AnthropicContentBlock`, `AnthropicMessagesResponse`, `ExtractedArticle`, `ManifestEntry`, `RecentTopic`, `FilterRuleField`, `TranslationDebugOutcome`, `SerializedWoTGraph`. Three transitively-used dev dependencies (`glob`, `js-yaml`, `@sentry/core`) explicitly added to `devDependencies`
  - **Comment hygiene**: ~30 paraphrase comments removed across 14 files (e.g. `// IC sync debounce: 3 seconds` directly above `setTimeout(..., 3_000)`; `// Reset banner on login` above the obvious effect; `// Register on canister` above `actor.registerPushSubscription`). 2 multi-paragraph blocks compressed (`hooks/useNotifications.ts` 4-line dismiss-duration WHY → 2 lines; `contexts/content/scoring.ts` 11-line concurrency/circuit-breaker note → 4 lines while keeping the load-bearing WHYs)
  - **Audited but already-clean**: zero circular dependencies, zero remaining `any`/`as any`/`Record<string, any>`, zero AI-stub functions, zero hardcoded values posing as dynamic, zero tautological tests, zero silent error swallowing in production code, zero unimplemented branches behind feature flags
- **D2A approve Err handling (LARP audit fix)**: `contexts/AgentContext.tsx::startAgent` awaited `ledger.icrc2_approve({...})` but discarded the result. The `try/catch` only caught network exceptions — when the ledger returned `{ Err: ApproveError }` (insufficient balance, allowance changed, expired, etc.), the code silently fell through and started the D2A agent in a half-broken state without notifying the user. `app/page.tsx:539` already handled the same call correctly with `if ("Err" in approveResult)` — that pattern now applied consistently in `AgentContext`. Two new tests in `__tests__/contexts/AgentContext.test.tsx` lock in both the Err path (notifies trusted-only mode) and the Ok path (does NOT notify trusted-only mode)

### Multi-Agent Round-2 Cleanup + Operational Tooling
- **423 suites / 7,488 tests** — zero failures, zero TypeScript errors, zero circular dependencies, all coverage thresholds met (`./lib/` held to a stricter 90/80/88/92 bar than the global 73/67/66/75)
- **Round-2 8-agent parallel pass**: a deeper sweep on top of the prior April cleanup. Eight subagents in isolated git worktrees, each producing a critical evaluation in `.claude/evaluations/2026-04-20/` and applying only HIGH-confidence recommendations. Two agents (cycles, defensive try-catch) found **nothing left to do** — confirmation the prior pass was thorough — and the remaining six produced focused changes:
  - **DRY** (`lib/api/kvStore.ts` new + `lib/utils/webgpu.ts` new): byte-for-byte duplicate `getKV()` lazy-singleton consolidated across `lib/api/rateLimit.ts` + `lib/api/dailyBudget.ts`; identical `isWebGPUAvailable` 3-line implementations in `lib/webllm/engine.ts` + `lib/mediapipe/engine.ts` consolidated; three near-identical `tryOllama`/`tryWebLLM`/`tryMediaPipe` cascade wrappers replaced with one parameterized `runLocalTier(engine, ...)` in `contexts/content/scoring.ts`; `ANTHROPIC_DEFAULT_MODEL` constant added to `lib/api/anthropic.ts` so the model literal exists in one place; new `parseJsonBody<T>()` helper in `lib/api/rateLimit.ts` adopted by `/api/analyze`, `/api/briefing/digest`, `/api/push/send`, `/api/push/token`
  - **Type consolidation**: `SourceSyncStatus` extracted to `lib/types/sources.ts` (was inlined 3× in `contexts/SourceContext.tsx`); doc comment differentiates it from `ContentSyncStatus` since the shapes are similar but the offline-queue path semantically diverges. `packages/d2a-client` SDK types intentionally NOT consolidated with host-app types — the SDK is publishable and must remain dependency-free
  - **Weak-type elimination** (4 sites): `lib/offline/actionQueue.ts` `payload: unknown` → discriminated union `QueuedAction = SaveEvaluationPayload | UpdateEvaluationPayload`, traced through `contexts/content/icSync.ts` `syncToIC` and `drainOfflineQueue`; `lib/api/anthropic.ts` `raw: unknown` → typed `AnthropicMessagesResponse` modelled on the documented Messages API contract; `lib/cache/urlExtract.ts` `data?: Record<string, unknown>` → concrete `ExtractedArticle`; `app/api/analyze/route.ts` `Promise<Record<string, unknown>>` → `Promise<ScoreParseResult & { tier: "claude" }>`. ~155 other `unknown` occurrences audited and confirmed legitimate (catch handlers, JSON-parse boundaries, validation guards, IDL factories)
  - **Legacy code**: `buildTranslationPrompt`'s `maxLength` legacy parameter removed — only callers were tests, JSDoc explicitly tagged it `legacy`
  - **Dead code (knip-verified)**: 3 type exports narrowed to module-private (`SettingsSubTab`, `ContentSource`, `ContentEvaluation`); knip's other "unused" findings systematically verified as false positives (jest pragma comments, React lifecycle methods, dynamic imports, generated declarations)
  - **Comment hygiene**: ~45 narration-style comments removed and ~50 multi-paragraph blocks compressed across 30 files. Kept every comment that encodes a non-obvious WHY (iOS Safari `onend` bug, IC LLM concurrency limit, kana validation rationale, BCP-47 normalization, SSRF redirect re-validation). Zero stubs/placeholders found
- **Observability gap closed (twice)**: 6 silent `.catch(() => null)` sites in `components/tabs/{Briefing,Sources}Tab.tsx` were replacing `response.json()` parse failures with `null` so users saw a fallback message but nothing was logged. Fix 1: added `console.warn("[<route>] error response not JSON:", err)` at each site. Fix 2 (post-audit honesty): caught my own mistake — the warns weren't reaching production because `Sentry.captureConsoleIntegration` was not wired up. Added it to all three Sentry configs (`sentry.client.config.ts`, `sentry.server.config.ts`, `sentry.edge.config.ts`) with `levels: ["warn", "error"]` so server-side route logs and client-side fetch errors now reach Sentry issue-grouping instead of dying in DevTools or Vercel stdout
- **Operational tooling**:
  - **`scripts/canister-backup.sh`** + `npm run canister:backup` — captures git SHA, working-tree status, live mainnet `module_hash` (read-only `dfx canister status`), source snapshot (`main.mo`, `types.mo`, `ledger.mo`, `aegis_backend.did`, `dfx.json`), and a reproduced wasm built from HEAD into `canister-backups/<timestamp>_<network>/`. End-to-end verified: locally-built wasm SHA-256 matches the live mainnet module hash, confirming HEAD reproduces what's deployed
  - **`scripts/canister-rollback.sh`** + `npm run canister:rollback <snapshot>` — wraps the manual `git checkout <sha> -- canisters/` + `dfx deploy --mode upgrade` workflow with snapshot integrity check, dry-run on a clean local replica via `git worktree` (catches M0169 stable-field-removal **before** mainnet), HEAD-vs-target diff display, required `'rollback'` literal-string confirmation (or `--confirm` for non-interactive), post-deploy live module hash verification. Refuses to proceed if stdin isn't a TTY and `--confirm` wasn't passed
  - **`loadtest/read-paths.k6.js`** + `loadtest/run.sh` + `npm run loadtest` — k6 baseline for public read paths (`/api/health`, `/api/feed/{rss,atom}`, `/api-docs`, `/openapi.yaml`, `/manifest.json`) with explicit thresholds (`http_req_duration p95 < 500ms`, `p99 < 1500ms`, error rate < 1%, `/api/health p95 < 300ms`). Runner refuses gracefully when k6 isn't installed and prints brew/Linux/Docker install instructions; warns + 5s grace before targeting `aegis-ai.xyz`. Not wired to CI (cost / rate-limit budget); designed for staging exercises and post-deploy verification
- **`.github/dependabot.yml`** — weekly npm + GitHub Actions patch updates, grouped (`patches`, `sentry`, `types`). Major bumps for `@dfinity/*`, `next`, `react`, `react-dom`, `tailwindcss` ignored — those are tracked separately because each requires deliberate breaking-change planning
- **Coverage thresholds enforced** in `jest.config.js` and CI — `npm run test:coverage` fails if global coverage drops below 73/67/66/75 (statements/branches/functions/lines, ~2 points below current measured) or if `./lib/` aggregate drops below 90/80/88/92. CI uploads the `coverage/` directory as a 14-day retention artifact. Per-module thresholds avoided after measuring per-file rate of false alarms
- **Honest verification cadence**: every fix verified with actual execution (`tsc --noEmit`, `npm run build`, `npm test`, threshold-enforced `npm run test:coverage`, script run-throughs that exercise every error path) before the next was started. One round of public self-correction when the initial Sentry console-capture omission was discovered post-fact
- **Repo hygiene**: `.claude/worktrees/` and `.claude/evaluations/` added to `.gitignore` (these are ephemeral Claude harness state and were being re-staged accidentally on `git add -A`). `canister-backups/` and `loadtest/last-run.json` ignored
...
- **Canister cycles probe** (`lib/ic/health.ts`): `/api/health` now calls `getCyclesBalance()` on the backend canister and flags `low` when balance falls below the 2T threshold mirrored from `canisters/aegis_backend/main.mo::CYCLES_THRESHOLD`. Result cached 60s in-process so uptime-monitor polling stays bounded (one IC query per minute per Vercel instance instead of one per request). `ROLLBACK.md` carries the full cycles runbook — thresholds, manual `dfx wallet send` top-up, freeze recovery
- **Feature flag framework** (`lib/featureFlags.ts`): typed env-driven flag registry evaluated once at process start. Five kill switches declared — `FEATURE_SCORING_CASCADE`, `FEATURE_TRANSLATION_CASCADE`, `FEATURE_BRIEFING_AGGREGATION`, `FEATURE_PUSH_SEND`, `X402_FREE_TIER_ENABLED`. Applied at server entry points: `/api/analyze` (heuristic-only short-circuit), `/api/translate` (503), `/api/d2a/briefing` global-path (503, per-principal unaffected), `/api/push/send` (503), `/api/d2a/briefing?preview=true` (x402 bypass). Flag state surfaced in `/api/health`'s `flags` field for operator visibility. Faster than a Vercel rollback for cost-spike incidents and doesn't invalidate edge cache
- **Sentry coverage completed**: `Sentry.captureException` added to server error paths in `/api/analyze`, `/api/push/send`, `/api/fetch/nostr`, `/api/fetch/rss`, `/api/fetch/url`, `/api/fetch/briefing`, `/api/fetch/twitter`, `/api/fetch/farcaster`, `/api/upload/image`, `/api/briefing/digest`, `/api/d2a/briefing`, `/api/d2a/briefing/changes`. API route 500/502 responses now flow to Sentry issue-grouping, not just Vercel logs
- **Canister pagination with owner index**: `sourceConfigOwnerIndex` transient map (rebuilt from stable state on `postupgrade`) maintained on every `saveSourceConfig` / `deleteSourceConfig`. Two new query methods — `getSourceConfigStatsPaginated(offset, limit)` and `getUserSourceConfigsPaginated(p, offset, limit)` — avoid the O(n) full-map scan the legacy methods perform. Verified end-to-end against `dfx deploy --mode upgrade` on a local replica: save, list, delete, upgrade, verify-index-survived. Legacy methods kept per M0169 rules
- **CI canister upgrade dry-run job**: new GitHub Actions job (`.github/workflows/ci.yml::canister-upgrade`) spins up a local replica, does a baseline `dfx deploy`, then `dfx deploy --mode upgrade`. Catches M0169 (stable field rename/remove) at PR time instead of at mainnet deploy time
- **Scoring cascade integration tests** (`__tests__/integration/scoringCascadeFallback.test.ts`): 14 tests covering every tier fallback — Ollama down → Server Claude, WebLLM down → heuristic, MediaPipe preferred over WebLLM, BYOK failure → IC LLM, IC circuit open → skip to Server Claude, IC transport error → Server Claude, all tiers fail → heuristic, cache hit short-circuits, userContext change invalidates cache. jsdom polyfills for `TextEncoder` / `AbortSignal.timeout` added
- **Bundle size — small wins**: `uuid` package removed in favor of `crypto.randomUUID()`, ICP ledger / Nostr publish / WoT graph builder moved to dynamic imports so unauthenticated landing visits don't pull the full crypto chain. Home route First Load JS: 415 kB → 414 kB. Larger reductions require replacing `nostr-tools` / reducing Sentry client surface, tracked but out of scope
- **Release tagging**: `scripts/release-tag.sh` + `npm run release:tag` — refuses dirty tree, non-main branch, diverged local main, or already-used version; creates annotated `vX.Y.Z` tag for stable references when rolling back. `ROLLBACK.md` documents the convention
- **Repo hygiene**: 11 historical audit `.md` files at repo root moved to `.claude/audits/` (also renamed `TODO-LARP-AUDIT*` → `LARP-AUDIT-0N.md` for consistency). `npm audit fix` applied — `follow-redirects` and `hono` CVEs resolved; remaining 10 moderate findings all documented as accepted risk in `SECURITY.md`
